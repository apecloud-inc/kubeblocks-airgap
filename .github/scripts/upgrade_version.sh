...
change_cloud_version() {
    ...
    for imageFile in "${imageFiles[@]}"; do
        ...
        if [[ "$UNAME" == "Darwin" ]]; then
            ...
            sed -i '' "s/^docker.io\/apecloud\/kubeblocks-console:.*[0-9]/docker.io\/apecloud\/kubeblocks-console:${CLOUD_VERSION}/" $image_file_path
...
generate_release_note() {
    ...
    mapfile -t addon_files < <(yq e 'with_entries(select(.value[0].type == "engine" and .value[0].isAddon == true)) | keys | .[]' "${MANIFESTS_FILE}" 2>/dev/null || true)
    for addon_name in "${addon_files[@]}"; do
        # 0.9 directory: damengdb uses -arm variant
        if [[ "${addon_name}" == "damengdb" ]]; then
            image_file_path=".github/images/0.9/${addon_name}-arm.txt"
        else
            image_file_path=".github/images/0.9/${addon_name}.txt"
        fi
        if [[ -f "${image_file_path}" ]]; then
            echo "add ${addon_name} (0.9) to release note"
            cat ${image_file_path} >> "$release_note_file"
        fi

        # 1.0 directory: oceanbase uses -arm variant
        if [[ "${addon_name}" == "oceanbase" ]]; then
            image_file_path=".github/images/1.0/${addon_name}-arm.txt"
        else
            image_file_path=".github/images/1.0/${addon_name}.txt"
        fi
        if [[ -f "${image_file_path}" ]]; then
            echo "add ${addon_name} (1.0) to release note"
            cat ${image_file_path} >> "$release_note_file"
        fi
    done
