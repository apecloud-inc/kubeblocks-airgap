...
FILES_TO_READ=("kubeblocks-enterprise.txt" "ape-local-csi-driver.txt" "cert-manager.txt" "clickhouse.txt" "damengdb-arm.txt" "elasticsearch-arm.txt" "ingress-nginx.txt" "kafka.txt" "gaussdb.txt" "influxdb.txt" "kubebench.txt" "metallb.txt" "metrics-server.txt" "minio.txt" "mongodb.txt" "mssql.txt" "mysql-arm.txt" "oceanbase-arm.txt" "postgres-operator.txt" "postgresql.txt" "qdrant.txt" "rabbitmq.txt" "redis.txt" "rocketmq.txt" "starrocks.txt" "tidb.txt" "vastbase.txt" "zookeeper.txt" "victoria-logs.txt" "victoria-metrics.txt" "spiderpool.txt"  )
