...
FILES_TO_READ=("kubeblocks-enterprise.txt" "ape-local-csi-driver.txt" "cert-manager.txt" "clickhouse.txt" "damengdb-arm.txt" "elasticsearch-arm.txt" "ingress-nginx.txt" "kafka.txt" "gaussdb.txt" "influxdb.txt" "kubebench.txt" "metallb.txt" "metrics-server.txt" "minio.txt" "mongodb-arm.txt" "mssql.txt" "mysql-arm.txt" "oceanbase.txt" "postgres-operator.txt" "postgresql.txt" "qdrant.txt" "rabbitmq.txt" "redis.txt" "rocketmq-images.txt" "starrocks-images.txt" "tidb.txt" "vastbase.txt" "zookeeper-images.txt" "victoria-metrics.txt" "spiderpool.txt"  )
