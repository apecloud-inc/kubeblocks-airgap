...
for DOCKER_IMAGE_FILE in "$@"; do
    ...
    nerdctl load -i "$DOCKER_IMAGE_FILE"
done
...
while IFS= read -r image; do
    ...
    if [[ "$tag_ret" != "0" ]]; then
        set +e
        nerdctl tag "$image_tmp" "$new_image"
        tag_ret_2=$?
        set -e
        if [[ "$tag_ret_2" != "0" ]]; then
            image_tmp_2=${image_tmp/docker.io\//}
            nerdctl tag "$image_tmp_2" "$new_image"
        fi
    fi
    # 推送镜像到指定的 registry
...
while IFS= read -r image; do
    ...
    nerdctl push "$new_image"
    echo "✅ $(tput -T xterm setaf 2) $new_image pushed successfully $(tput -T xterm sgr0)"
done < <(nerdctl images --format '{{.Repository}}:{{.Tag}}' | grep -E 'apecloud/kubernetes-airgap:|apecloud/calico-airgap:|labring/helm:|labring/metrics-server:|labring/openebs:|labring/coredns:' || true)
